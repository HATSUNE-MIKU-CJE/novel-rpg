...
| 阶段 | 内容 | 验收 |
|------|------|------|
| M1 骨架 | 脚手架 + SQLite schema + API 配置页 + 基础对话打通 + 宏引擎 | 手机跑通「配置 API → 建存档 → 对话有回应」 ✅已完成 |
| M2 预设引擎 | ST 预设导入 + prompts 顺序渲染 + XML 输出解析 + 世界书极简注入 | 导入梦鲸思客V4 预设后，对话输出 dream_plot 且正文/场景卡正常渲染 ✅已完成 |
| M3 内置化 | 内置梦鲸思客精简预设 + 分组开关面板 + 内置默认节点 + token 统计 + 输出上限自由调 + 上下文压缩 + 对话搜索 + 重发/模式切换 + **宝宝化 v2.1** | ✅已完成（自测 49/49 + 浏览器端到端 13/13） |
| M4 自动写 | 事实提取 + 自动笔记簿 + 审阅面板 | ✅已完成（浏览器端到端 20/20）：整理→角色卡→待审阅→接受/拒绝 全链路 |
| M5 面板增强 | 关系图可视化 + 存档×世界书绑定管理 + 规范导出页 + 变量查看器 | 面板可查角色/关系；可导出规范文档和世界书 JSON |
| M6 打磨 | 功能扩展候选（主题切换等）+ 整库备份 + APK 发布 | 换机不丢档 |
