...
> v3.5 更新：规范导出升级为 **v2 八类卡体系**（`kind` + 各类卡 `payload` + `hook`/`timeline`/`isMain`），
> 完整字段说明与 JSON Schema 由 App 内「设置 → 世界书格式规范」导出（源码：`src/engine/specExport.ts`）。
> 下方 v1 定义保留为历史参考；导入器对 v1/v2/ST 三种格式均自动识别。
...
| v3.5 导入与规范 | ST 导入清洗（宏/HTML 标签/--- 分隔线剥离，人物卡自动生成 payload 与触发词）+ 绑定即自动接入人物卡（含存量自愈、幂等）+ 世界书格式规范导出升级 v2（八类卡 + 各类 payload + hook/timeline/isMain + JSON Schema + 完整示例） | ✅已完成 |
