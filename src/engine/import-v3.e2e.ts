...
// v3.5.2：markdown 结构与英文音译注释（斗罗真实格式）
check('markdown 标题剥离', cleanImportedContent('# 角色档案：唐三 (Tang San)') === '角色档案：唐三', JSON.stringify(cleanImportedContent('# 角色档案：唐三 (Tang San)')))
check('markdown 二级标题剥离', cleanImportedContent('## 【基本信息】') === '【基本信息】')
check('缩进标题剥离', cleanImportedContent('  ## 【基本信息】') === '【基本信息】', JSON.stringify(cleanImportedContent('  ## 【基本信息】')))
check('缩进标题+列表剥离', !cleanImportedContent('\n  ## 【基本信息】\n  - **全名**: 唐三').includes('#') && !cleanImportedContent('\n  ## 【基本信息】\n  - **全名**: 唐三').includes('*'))
check('markdown 列表剥离', cleanImportedContent('- **全名**: 唐三') === '全名: 唐三', JSON.stringify(cleanImportedContent('- **全名**: 唐三')))
check('markdown 加粗剥离', !cleanImportedContent('**粗体** 与 *斜体* 与 `代码`').includes('*') && !cleanImportedContent('**粗体** 与 *斜体* 与 `代码`').includes('`'), JSON.stringify(cleanImportedContent('**粗体** 与 *斜体* 与 `代码`')))
check('英文音译注释剥离', !cleanImportedContent('# 角色档案：唐三 (Tang San)').includes('Tang San'))
check('真实斗罗头部清洗', cleanImportedContent('# 角色档案：唐三 (Tang San)\n\n  ## 【基本信息】\n  - **全名**: 唐三') === '角色档案：唐三\n\n【基本信息】\n全名: 唐三', JSON.stringify(cleanImportedContent('# 角色档案：唐三 (Tang San)\n\n  ## 【基本信息】\n  - **全名**: 唐三')))
check('中文括号保留', cleanImportedContent('（顶级兽武魂）不影响').includes('（顶级兽武魂）'))
